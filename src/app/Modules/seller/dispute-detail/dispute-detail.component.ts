...
import { RealUpdateService, CommonUtilsService, SellerService, NotificationService } from '../../../core/_services';
//import models
...
export class DisputeDetailComponent implements OnInit {
  ...
  readonly breadcrumbs: any[] = [{ page: 'Home', link: '/seller/home' }, { page: "Dispute Listing", link: '/seller/disputes' }, { page: " Dispute Detail", link: '' }]
  constructor(private _location :Location,private commonUtilService: CommonUtilsService, private sellerService: SellerService, private activatedRoute: ActivatedRoute, private fb: FormBuilder) {
  ...
  getDisputeDetail(){
    this.messages = [];
    this.disputeDetail = {};
  this.sellerService.getDisputeDetail({ _id: this.activatedRoute.snapshot.params.id }).pipe(untilDestroyed(this)).subscribe(response => {
...
/**
 * this will save the dispute message from seller to admin 
 */
  sendMessage():void {
    ...
    this.sellerService.saveDisputeMessage(obj).subscribe(response => {
